...
class StonehengeState(GameState):
    ...
    def __init__(self, is_p1_turn: bool, side_length: int) -> None:
        """
        Initialise variables based on Player 1's turn.
        ...
        self.cells = {}
        self.ley_line_count = {}
        num_cells = (self.side_length ** 2 + 5 * self.side_length) // 2
        moves = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
        for i in range(num_cells):
            if i >= num_cells - self.side_length and self.side_length < 5:
                self.cells['c' + str(i + 2)] = moves[i]
            else:
                self.cells['c' + str(i + 1)] = moves[i]
    ...
    def get_possible_moves(self) -> List[str]:
        ...
        for leyline in self.ley_line_state:
            if self.ley_line_state[leyline] == 1:
                ...
                count_p2 += 1
        if (count_p1 >= len(self.ley_line_state) // 2 or
                count_p2 >= len(self.ley_line_state) // 2) and \
                even_amount:
            self.is_true = True
        elif (count_p1 > len(self.ley_line_state) // 2 or
              count_p2 > len(self.ley_line_state) // 2) and \
                not even_amount:
            self.is_true = True
        if not self.is_true:
    ...
    def make_move(self, move: Any) -> "StonehengeState":
        ...
        for leyline in self.ley_line:
            ...
            if same_move in self.ley_line[leyline] and self.p1_turn:
                new_state.ley_line_count[leyline][0] += 1
            elif same_move in self.ley_line[leyline] and not self.p1_turn:
